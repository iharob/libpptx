...
typedef struct pptx_ct_p_group_shape_non_visual_s {
...
pptx_ct_p_group_shape_non_visual *
pptx_ct_p_group_shape_non_visual_new(xmlNode *node)
{
    pptx_ct_p_group_shape_non_visual *obj;
    ...
    xpath = pptx_xml_find_node(node, (const xmlChar *) "p:cNvGrpSpPr");
    if (xpath != NULL) {
        xmlNodeSet *nodes;
        nodes = xpath->nodesetval;
        if ((nodes->nodeNr != 1) || (nodes->nodeNr < -1)) {       
...
pptx_ct_p_group_shape_non_visual_get_c_nv_pr(const pptx_ct_p_group_shape_non_visual *const obj)
{
    return obj->c_nv_pr;
}

pptx_ct_a_non_visual_group_drawing_shape_props *
pptx_ct_p_group_shape_non_visual_get_c_nv_grp_sp_pr(const pptx_ct_p_group_shape_non_visual *const obj)
{
    return obj->c_nv_grp_sp_pr;
}

pptx_ct_p_application_non_visual_drawing_props *
pptx_ct_p_group_shape_non_visual_get_nv_pr(const pptx_ct_p_group_shape_non_visual *const obj)
{
    return obj->nv_pr;
}

int32_t
pptx_ct_p_group_shape_non_visual_get_index(pptx_ct_p_group_shape_non_visual *obj)
{
    return obj->index;
}

void
pptx_ct_p_group_shape_non_visual_free(pptx_ct_p_group_shape_non_visual *obj)
{
    if (obj == NULL)
        return;
