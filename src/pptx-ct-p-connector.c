...
typedef struct pptx_ct_p_connector_s {
    pptx_ct_p_connector_non_visual *nv_cxn_sp_pr;
...
pptx_ct_p_connector *
pptx_ct_p_connector_new(xmlNode *node)
{
    pptx_ct_p_connector *obj;
    ...
    xpath = pptx_xml_find_node(node, (const xmlChar *) "p:nvCxnSpPr");
    if (xpath != NULL) {
        xmlNodeSet *nodes;
        ...
        if ((nodes->nodeNr != 1) || (nodes->nodeNr < -1)) {       
            ...
            obj->nv_cxn_sp_pr = pptx_ct_p_connector_non_visual_new(nodes->nodeTab[0]);
        }
...
pptx_ct_p_connector_non_visual *
pptx_ct_p_connector_get_nv_cxn_sp_pr(const pptx_ct_p_connector *const obj)
{
    return obj->nv_cxn_sp_pr;
}

pptx_ct_a_shape_properties *
pptx_ct_p_connector_get_sp_pr(const pptx_ct_p_connector *const obj)
{
    return obj->sp_pr;
}

pptx_ct_a_shape_style *
pptx_ct_p_connector_get_style(const pptx_ct_p_connector *const obj)
{
    return obj->style;
}

pptx_ct_p_extension_list_modify *
pptx_ct_p_connector_get_ext_lst(const pptx_ct_p_connector *const obj)
{
    return obj->ext_lst;
}

int32_t
pptx_ct_p_connector_get_index(pptx_ct_p_connector *obj)
{
    return obj->index;
}

void
pptx_ct_p_connector_free(pptx_ct_p_connector *obj)
{
    if (obj == NULL)
        return;
    pptx_ct_p_connector_non_visual_free(obj->nv_cxn_sp_pr);
