...
typedef struct pptx_ct_p_picture_s {
    pptx_ct_p_picture_non_visual *nv_pic_pr;
...
pptx_ct_p_picture *
pptx_ct_p_picture_new(xmlNode *node)
{
    pptx_ct_p_picture *obj;
    ...
    xpath = pptx_xml_find_node(node, (const xmlChar *) "p:nvPicPr");
    if (xpath != NULL) {
        xmlNodeSet *nodes;
        ...
        if ((nodes->nodeNr != 1) || (nodes->nodeNr < -1)) {       
            ...
            obj->nv_pic_pr = pptx_ct_p_picture_non_visual_new(nodes->nodeTab[0]);
        }
    ...
    xpath = pptx_xml_find_node(node, (const xmlChar *) "p:blipFill");
    if (xpath != NULL) {
        xmlNodeSet *nodes;
        nodes = xpath->nodesetval;
        if ((nodes->nodeNr != 1) || (nodes->nodeNr < -1)) {       
...
pptx_ct_p_picture_get_nv_pic_pr(const pptx_ct_p_picture *const obj)
{
    return obj->nv_pic_pr;
}

pptx_ct_a_blip_fill_properties *
pptx_ct_p_picture_get_blip_fill(const pptx_ct_p_picture *const obj)
{
    return obj->blip_fill;
}

pptx_ct_a_shape_properties *
pptx_ct_p_picture_get_sp_pr(const pptx_ct_p_picture *const obj)
{
    return obj->sp_pr;
}

pptx_ct_a_shape_style *
pptx_ct_p_picture_get_style(const pptx_ct_p_picture *const obj)
{
    return obj->style;
}

pptx_ct_p_extension_list_modify *
pptx_ct_p_picture_get_ext_lst(const pptx_ct_p_picture *const obj)
{
    return obj->ext_lst;
}

int32_t
pptx_ct_p_picture_get_index(pptx_ct_p_picture *obj)
{
    return obj->index;
}

void
pptx_ct_p_picture_free(pptx_ct_p_picture *obj)
{
    if (obj == NULL)
        return;
    pptx_ct_p_picture_non_visual_free(obj->nv_pic_pr);
