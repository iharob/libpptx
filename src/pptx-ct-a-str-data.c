...
typedef struct pptx_ct_a_str_data_s {
    pptx_uint pt_count;
...
pptx_ct_a_str_data *
pptx_ct_a_str_data_new(xmlNode *node)
{
    pptx_ct_a_str_data *obj;
...
pptx_uint 
pptx_ct_a_str_data_get_pt_count(const pptx_ct_a_str_data *const obj)
{
    return obj->pt_count;
}

pptx_ct_a_str_val **
pptx_ct_a_str_data_get_pt(const pptx_ct_a_str_data *const obj)
{
    return obj->pt;
}

pptx_ct_a_extension_list *
pptx_ct_a_str_data_get_ext_lst(const pptx_ct_a_str_data *const obj)
{
    return obj->ext_lst;
}

int32_t
pptx_ct_a_str_data_get_index(pptx_ct_a_str_data *obj)
{
    return obj->index;
}

void
pptx_ct_a_str_data_free(pptx_ct_a_str_data *obj)
{
    if (obj == NULL)
        return;
    if (obj->pt != NULL) {
